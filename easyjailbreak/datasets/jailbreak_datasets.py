...
class JailbreakDataset(torch.utils.data.Dataset):
    ...
    def save_to_jsonl(self, path='data.jsonl', call_info=None):
        """
        ...
        :param call_info: (optional) dictionary of call counts to include in the output
        """
        import jsonlines
        ...
        with jsonlines.open(path, mode='w') as writer:
            for instance in self._dataset:
                data = {
                    ...
                    'call_info': getattr(instance, 'call_info', None),
                    'refinement_history': getattr(instance, 'refinement_history', None),
                }
                writer.write(data)
