...
r"""
EasyJailbreak IntrospectGenerationAggregateMaxConcatenate class
===============================================================
"""
__all__ = ["IntrospectGenerationAggregateMaxConcatenate", "random_string", "extract_json", "conv_template"]
class IntrospectGenerationAggregateMaxConcatenate(MutationBase):
    ...
    >>> from easyjailbreak.mutation.generation.IntrospectGenerationAggregateMaxConcatenate import IntrospectGenerationAggregateMaxConcatenate
    ...
    >>> mutator = IntrospectGenerationAggregateMaxConcatenate(attack_model, system_prompt)
    ...
    def __init__(self, model,system_prompt, branching_factor=5, keep_last_n=3, max_n_attack_attempts=5,
                 attr_name="jailbreak_prompt", prompt_format=None, selection_strategy=None):
        """
        ...
        """
        super().__init__(model,system_prompt, branching_factor, keep_last_n, max_n_attack_attempts, attr_name, prompt_format, selection_strategy)
        self.model = model
    ...
    # Samples k thoughts with the highest scores
    def sample_max_score(self, thoughts, k=1):
        sorted_thoughts = sorted(thoughts, key=lambda x: x['score'], reverse=True)
        if len(thoughts) < k:
            print("Warning: k is larger than the number of available thoughts.")
            return sorted_thoughts
        return sorted_thoughts[:k] # List of k thoughts with the highest scores.

    def _get_mutated_instance(self, instance, conversation_history, *args, **kwargs)->List[Instance]:
        r"""
        ...
        :conversation_history (list): A list of previous conversational exchanges (ICLs across all branches)
        ...
        if self.selection_strategy == 'baseline':# or len(instance.eval_results)<2:
            conv.messages = conv.messages[-self.keep_last_n * 2:]
        else:
            print("GAP filtering")
            num_messages_to_filter = self.keep_last_n
            sampled_conversations = self.sample_max_score(conversation_history, k=num_messages_to_filter)

            sampled_conversations_messages = [conv['conversation'] for conv in sampled_conversations]
            sampled_conversations_messages_flattened = [x for sublist in sampled_conversations_messages for x in sublist]

            print(sampled_conversations_messages_flattened)
            print(conv.messages)
            if len(sampled_conversations_messages_flattened)>0:
                print("GAP filtering...sampling")
                conv.messages = sampled_conversations_messages_flattened + conv.messages[-self.keep_last_n*2:] ## msg-last  # conv.messages = [conv.messages[-self.keep_last_n*2] + sampled_conversations_messages_flattened] ## msg-first
            else:
                print("GAP filtering...no sampling")
                conv.messages = conv.messages[-self.keep_last_n * 2:]
    ...
    def get_attack(self, model, conv, prompt, query, reference_response):
        ...
        for _ in range(max_n_attack_attempts):
            ...
            if isinstance(model, HuggingfaceModel) or isinstance(model, MistralModel):
...
def extract_json(s):
    ...
    try:
        ...
        if not all(x in parsed for x in ["improvement","prompt"]):
            return None, None
        # Check if mutator outputs new prompt
        if ('[new prompt]' in parsed) or ('[new prompt]' in json_str):
            return None, None
